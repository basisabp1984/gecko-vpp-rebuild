...
import { Battery, Sun, Search } from "lucide-react";
...
export default function StorageAssetsPage() {
  ...
  const fleet = useMemo(() => {
    ...
    let energy = 0;
    let n = 0;
    for (const a of rows.filter((r) => r.asset_class === "УЗЕ")) {
      cap += parseFloat(a.capacity_mw);
      if (a.storage_capacity_mwh) {
        energy += parseFloat(a.storage_capacity_mwh);
      }
      n += 1;
    }
    return { cap, energy, n };
    ...
    <div className="flex flex-col gap-5">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-sm text-text-muted">
          Парк установок зберігання енергії та допоміжна генерація. Натисніть
      ...
      {fleet.n > 0 && (
        <section className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          <FleetTile
            label="УЗЕ-парк (потужність)"
            value={`${formatNumber(fleet.cap, 1)} МВт`}
          />
          <FleetTile
            label="УЗЕ-парк (ємність)"
            value={`${formatNumber(fleet.energy, 1)} МВт·год`}
          />
          <FleetTile label="К-сть батарей" value={String(fleet.n)} />
        </section>
      )}
            ...
            <table className="w-full text-sm">
              <thead className="bg-bg-subtle text-xs uppercase tracking-wide text-text-muted">
                <tr>
                  ...
                  <th className="px-4 py-2.5 text-right">Ємність</th>
              ...
              <tbody>
                {filtered.map((a, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right font-mono text-text-body">
                      {a.storage_capacity_mwh
                        ? `${formatNumber(a.storage_capacity_mwh, 1)} МВт·год`
                        : "—"}
                    </td>
...
function FleetTile({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-border bg-bg-card p-3 shadow-card">
      <div className="text-[10px] uppercase tracking-wide text-text-muted">
        {label}
      </div>
      <div className="mt-1 text-base font-bold text-text-heading">{value}</div>
    </div>
  );
}
