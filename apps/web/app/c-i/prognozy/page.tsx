...
export default function CIForecastsPage() {
  useEffect(() => {
  ...
  const [active, setActive] = useState<ForecastKind>("load");
  ...
  const submissions = useAPI<SubmissionRow[]>(
  ...
  const chartData = useMemo(() => {
    ...
    for (const r of ac) {
      const key = r.interval_start;
      actMap.set(key, (actMap.get(key) ?? 0) + parseFloat(r.actual_mwh));
    }
  ...
  const mut = useMutation<SubmitResult, ApiError, void>({
    mutationFn: async () => {
      const eic = consumerAsset?.resource_eic ?? "10W-UA-ASSET-009";
      const cap = consumerAsset
        ? parseFloat(consumerAsset.capacity_mw)
        : 8;
      const payload = {
        delivery_date: todayISO(),
        resource_eic: eic,
        hourly_volumes_mwh: Array.from({ length: 24 }, (_, i) => {
          // Industrial load — flat-ish around 60–80% capacity 8:00–18:00
          const t = i;
          const base = t >= 8 && t <= 18 ? cap * 0.7 : cap * 0.35;
          const jitter = (Math.sin(t * 0.7) + 1) * cap * 0.05;
    ...
    onSuccess: (d) => {
      toast.push({
        tone: "success",
        title: "Прогноз споживання подано",
        description: `ID: ${d.submission_id ?? d.id ?? "—"} · статус ${d.status ?? "—"}`,
      });
      qc.invalidateQueries({
        predicate: (q) =>
          typeof q.queryKey?.[1] === "string" &&
          (q.queryKey[1] as string).startsWith(
            "/api/v1/regulatory/submissions",
          ),
      });
    },
      ...
      <header className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-text-muted">
            Денний прогноз навантаження та власної генерації. Refined vs
            primary з фактом телеметрії.
          </p>
        </div>
        <button
          type="button"
