...
import { Sun, Wind, Battery, Factory, Search } from "lucide-react";
import { useAPI } from "@/lib/api";
import { AssetDrawer, type AssetSummary } from "@/components/AssetDrawer";
import { formatNumber } from "@/lib/format";

type AssetClass = "all" | "СЕС" | "ВЕС" | "ГПУ" | "УЗЕ";

const FILTERS: { id: AssetClass; label: string; icon?: React.ReactNode }[] = [
  { id: "all", label: "Всі" },
  { id: "СЕС", label: "СЕС", icon: <Sun size={14} /> },
  { id: "ВЕС", label: "ВЕС", icon: <Wind size={14} /> },
  { id: "ГПУ", label: "ГПУ", icon: <Factory size={14} /> },
  { id: "УЗЕ", label: "УЗЕ", icon: <Battery size={14} /> },
];

export default function ProducerAssetsPage() {
    ...
    <div className="flex flex-col gap-5">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-sm text-text-muted">
          Перелік СЕС, ВЕС, ГПУ та УЗЕ у портфелі. Натисніть рядок для деталей
          активу та телеметрії.
        </p>
      </header>
...
function ClassBadge({ cls }: { cls: string }) {
  const map: Record<string, { label: string; tone: string }> = {
    ...
    ВЕС: { label: "ВЕС", tone: "bg-info/15 text-info" },
    ГПУ: { label: "ГПУ", tone: "bg-alert/15 text-alert" },
